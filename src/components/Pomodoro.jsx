...
import { Play, Pause, RotateCcw, Settings2, Bell, BellOff } from "lucide-react";
...
function ProgressRing({ mode, secondsLeft, totalSeconds, running }) {
  ...
  return (
    <div
      className={`relative h-52 w-52 shrink-0 ${running ? "animate-pulseRing" : ""}`}
    >
      <svg
        width={size}
        height={size}
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <span className="text-xs text-ghost-muted mt-1">
  ...
  return (
    <div className="rounded-2xl bg-ink-surface border border-ink-line p-6 flex flex-col items-center gap-6">
      <div className="flex items-center gap-2">
        {permission === "granted" ? (
          <span className="flex items-center gap-1 text-xs text-ghost-muted">
            <Bell size={14} className="text-mint" />
          ...
          <button
            ...
            className="text-xs text-volt bg-volt/10 rounded-full px-3 py-1 hover:bg-volt/20 transition-colors flex items-center gap-1"
          >
            <Bell size={14} />
      ...
      <div className="flex flex-wrap justify-center gap-2">
        {MODES.map((m) => (
          <button
            ...
            `}
          >
            {POMODORO_LABELS[m]} {durations[m]}m
          </button>
        ))}
        <button
      ...
      {showCustom && (
        <div className="flex flex-wrap justify-center gap-3 -mt-2">
          {MODES.map((m) => (
            <label
              key={m}
              className="flex flex-col items-center gap-1 text-[10px] text-ghost-faint"
            >
              {POMODORO_LABELS[m]}
              <input
                type="number"
                ...
                value={durations[m]}
                onChange={(e) =>
                  setCustomDuration(m, Math.max(1, Number(e.target.value) || 1))
                }
      ...
      <p className="text-xs text-ghost-faint font-mono">
        Session {sessionIndex} of {SESSIONS_BEFORE_LONG_BREAK} · Next:{" "}
        {POMODORO_LABELS[nextMode]}
      </p>

      <div className="grid grid-cols-3 gap-3 w-full pt-2 border-t border-ink-line">
        <div className="flex flex-col items-center py-2">
          <span className="font-display text-lg font-700 text-ghost">
            {todaySessions}
          </span>
          <span className="text-[10px] text-ghost-muted mt-0.5">
            Sessions today
          </span>
        </div>
        <div className="flex flex-col items-center py-2">
